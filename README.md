...
* Require the SonarCloud Code Analysis check before merging to the default branch in public repositories
...
require `SonarCloud Code Analysis`; repository-specific lint, versioning, build, test,
and validation checks are merged into the same status-check rule. Private repositories
do not inherit the SonarCloud gate.
